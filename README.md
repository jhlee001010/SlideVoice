...
2. [빠른 시작](#빠른-시작)
3. [Windows](#windows)
4. [macOS / Linux (WSL 포함)](#macos--linux-wsl-포함)
5. [대본 작성 방법](#대본-작성-방법)
6. [목소리 설정](#목소리-설정)
7. [주요 옵션](#주요-옵션)
8. [동작 방식](#동작-방식)
...
결과물 느낌이 확 달라지는 건 아래 5개입니다. 나머지는 몰라도 됩니다.

- **`--voice`** — 목소리 자체를 바꿉니다. 사람마다 선호가 완전히 다르니 [목소리 설정](#목소리-설정)에서 직접 들어보고 고르세요.
- **`--speed`** (기본 `1.05`) — 말하는 속도. 강의처럼 내용이 길면 `1.15~1.2` 정도로 올리는 걸 추천합니다 (늘어지는 느낌이 줄어듦).
- **`--sentence-pause`** (기본 `0.4`) — 한 슬라이드 안에서 문장과 문장 사이 숨 쉬는 간격. 값을 올리면 또박또박, 내리면 빠르게 이어 말하는 느낌이 됩니다.
- **`--pad`** (기본 `0.4`) — 슬라이드가 끝나고 다음 슬라이드로 넘어가기 전 여백. 너무 짧으면 슬라이드가 바뀌자마자 다음 말이 시작돼서 정신없게 느껴질 수 있습니다.
- **`--steps`** (기본 `8`) — 음질과 속도를 맞바꾸는 값입니다. **직접 같은 문장으로 시간을 재본 결과**는 아래와 같습니다 (문장 1개 기준):

  | `--steps` 값 | 합성 시간 | 비고 |
  |---|---|---|
  | `2` | 약 1.2초 | 제일 빠르지만 음질이 다소 뭉개짐 |
  | `4` | 약 1.5초 | 빠름, 음질 약간 아쉬움 |
  | `8` (기본) | 약 2.2초 | 속도/음질 균형 — 대부분 이 값이면 충분 |
  | `16` | 약 2.3초 | 8과 시간은 비슷한데 음질은 조금 더 안정적 |
  | `32` | 약 3.9초 | 확실히 느려지는 대신 음질 향상 |
  | `50` | 약 5.1초 | 가장 느림, 미세한 차이만 남음 |

  슬라이드 수가 많을수록 이 시간이 그대로 누적되니(슬라이드 30장이면 steps=8 대비 steps=32는 대략 +50초 이상 더 걸림), 평소엔 기본값을 쓰다가 **최종 완성본만 품질을 올리고 싶을 때** `--steps 16`이나 `32` 정도로 다시 만들어보는 걸 추천합니다.

```powershell
venv\Scripts\python ppt2video.py --pptx deck.pptx --out output.mp4 --voice M3 --speed 1.15 --sentence-pause 0.5 --steps 16
```

<details>
<summary>전체 옵션 목록</summary>

...
**PPT(파워포인트)나 PDF 슬라이드 + 발표 대본(텍스트)만 있으면, 사람 목소리로 읽어주는 나레이션 영상(mp4 파일)을 자동으로 만들어주는 프로그램입니다.**

- 직접 마이크로 녹음할 필요 없이, AI 목소리(TTS = Text-to-Speech, "글자를 읽어서 소리로 바꿔주는 기술")가 대본을 대신 읽어줍니다.
- 강의 영상, 발표 영상, 인강 자료 같은 걸 목소리 녹음 없이 빠르게 만들 때 씁니다.
- 완전히 무료로 쓸 수 있는 오픈소스 프로그램만 사용합니다 (별도 유료 결제 없음).
...
**전체 흐름은 이렇습니다:**
1. 만들어둔 PPT(또는 PDF) 파일을 이 프로그램 폴더에 넣는다.
2. 각 슬라이드마다 읽어줄 대본을 준비한다 (PPT의 "발표자 노트"에 미리 써두면 자동으로 읽어옵니다).
3. 명령어 한 줄을 실행한다.
4. 잠시 기다리면 완성된 mp4 영상 파일이 나온다.
